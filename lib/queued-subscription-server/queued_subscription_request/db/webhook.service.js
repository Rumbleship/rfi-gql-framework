...
let WebhookServiceSequelize = class WebhookServiceSequelize extends sequelize_base_service_1.SequelizeBaseService {
    ...
    async addWebhook(config, input, opts) {
        if (this.can({
            ...
            const webhook = await this.addAuthorizationFiltersAndWrapWithTransaction({ opts: { ...opts, ...{ lockLevel: relay_interface_1.NodeServiceLock.UPDATE } } }, async (optionsWithTransactionAndAuth) => {
                ...
                webhookUpdate.topic_name = `${pubSubConfig.topicPrefix}_${config.serviceName}webhooks_${webhookRelay.division_id}_${webhookRelay.id.toString()}`;
                webhookUpdate.subscription_name = webhookUpdate.topic_name;
                await this.createTopicAndSubscriptionForGooglePubSub(gcpConfig, webhookUpdate.topic_name, webhookUpdate.subscription_name, webhookUpdate.subscription_url);
    ...
    async removeWebhook(webhookId, opts) {
        return await this.addAuthorizationFiltersAndWrapWithTransaction({ opts: { ...opts, ...{ lockLevel: relay_interface_1.NodeServiceLock.UPDATE } } }, async (optionsWithTransactionAndAuth) => {
            const webhook = await this.getOne(new oid_1.Oid(webhookId));
    ...
    async addSubscription(webhookId, input, opts) {
        return await this.addAuthorizationFiltersAndWrapWithTransaction({ opts: { ...opts, ...{ lockLevel: relay_interface_1.NodeServiceLock.UPDATE } } }, async (optionsWithTransactionAndAuth) => {
            ...
            const qsrService = this.getServiceFor(gql_1.QueuedSubscriptionRequest);
    ...
    async removeSubscription(webhookId, subscriptionId, opts) {
        ...
        return await this.addAuthorizationFiltersAndWrapWithTransaction({ opts: { ...opts, ...{ lockLevel: relay_interface_1.NodeServiceLock.UPDATE } } }, async (optionsWithTransactionAndAuth) => {
            ...
            const qsrRelay = await qsrService.update({ id: subscriptionId, active: false }, optionsWithTransactionAndAuth);
    ...
    async getWebhookSubscriptionsFor(aWebhook, filter, opts) {
        return super.getAssociatedMany(aWebhook, 'webhookSubscriptions', filter, gql_1.QueuedSubscriptionRequest, gql_1.QueuedSubscriptionRequestEdge, gql_1.QueuedSubscriptionRequestConnection, opts);
    }
    async createTopicAndSubscriptionForGooglePubSub(gcpConfig, topic_name, subscription_name, subscription_url) {
        const googlePubSub = new pubsub_1.PubSub(gcpConfig.Auth);
        const topic = await gcp_helpers_1.gcpGetTopic(googlePubSub, topic_name);
        await gcp_helpers_1.gcpCreatePushSubscription(topic, subscription_name, subscription_url);
    }
};
