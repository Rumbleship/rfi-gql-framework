...
const create_date_filter_1 = require("./create-date-filter");
/**
        ...
        if (typeof key === 'string') {
            if (key.endsWith(range_suffix)) {
                ...
                if (range instanceof gql_1.DateRange) {
                    filter = create_date_filter_1.create_date_filter(filter, key.substr(0, key.length - range_suffix.length) + date_suffix, range);
