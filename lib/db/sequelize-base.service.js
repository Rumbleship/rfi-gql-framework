...
let SequelizeBaseService = class SequelizeBaseService {
    ...
    async getAll(filterBy, options) {
        ...
        if (this.ctx.authorizer.can(acl_1.Actions.QUERY, filter, [this.permissions])) {
    ...
    async getOne(oid, options) {
        ...
        if (this.ctx.authorizer.can(acl_1.Actions.QUERY, instance, [this.permissions])) {
    ...
    async create(data, options) {
        if (this.ctx.authorizer.can(acl_1.Actions.CREATE, data, [this.permissions])) {
            const sequelizeOptions = this.convertServiceOptionsToSequelizeOptions(options);
    ...
    async update(data, options) {
        if (data.id) {
            if (this.ctx.authorizer.can(acl_1.Actions.UPDATE, data, [this.permissions])) {
