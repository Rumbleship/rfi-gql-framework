...
let SequelizeBaseService = class SequelizeBaseService {
    ...
    can(action, authorizable, options) {
        return (options &&
            options.transaction &&
    ...
    async getAll(filterBy, options) {
        ...
        if (this.can(acl_1.Actions.QUERY, filter, options)) {
    ...
    async count(filterBy) {
        const { ...filter } = filterBy;
        if (this.can(acl_1.Actions.QUERY, filter)) {
    ...
    async getOne(oid, options) {
        ...
        if (this.can(acl_1.Actions.QUERY, instance, options)) {
    ...
    async publishLastKnownState(oid) {
        ...
        if (this.can(acl_1.Actions.QUERY, instance)) {
            gql_pubsub_sequelize_engine_1.publishCurrentState(instance);
        }
        throw new acl_1.RFIAuthError();
    }
    async create(data, options) {
        if (this.can(acl_1.Actions.CREATE, data, options)) {
            const sequelizeOptions = this.convertServiceOptionsToSequelizeOptions(options);
    ...
    async update(data, options) {
        if (data.id) {
            if (this.can(acl_1.Actions.UPDATE, data, options)) {
