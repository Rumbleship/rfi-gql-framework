        ...
        if (modelClass.isInitialized) {
            if (!SequelizeBaseService_1.hooksMap.has(modelClass)) {
                ...
                modelClass.addHook('beforeFind', (findOptions) => {
                    ...
                    if (authorizeContext.authApplied || ((_a = authorizeContext.nodeServiceOptions) === null || _a === void 0 ? void 0 : _a.skipAuthorizationCheck) ||
                        authorizeContext.service.getContext().authorizer.inScope(acl_1.Scopes.SYSADMIN)) {
                        return;
                    }
    ...
    gqlFromDbModel(dbModel) {
        ...
        for (const key in gqlObject) {
            if (gqlObject.hasOwnProperty(key)) {
                if (gqlObject[key] instanceof sequelize_typescript_1.Model) {
                    const service = this.getServiceForDbModel(gqlObject[key]);
                    gqlObject[key] = service.gqlFromDbModel(gqlObject[key]);
                }
            }
        }
        return gqlObject;
