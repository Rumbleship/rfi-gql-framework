...
let SequelizeBaseService = class SequelizeBaseService {
    constructor(apiClass, edgeClass, connectionClass, model, sequelizeDataloaderCtx, apiClassFactory) {
    ...
    gqlFromDao(dao) {
        if (this.apiClassFactory) {
            ...
            return model_to_class_1.modelToClass(this, this.apiClass, dao);
        }
    }
    getServiceFor(cls) {
    ...
    async getAll(filterBy, paranoid = true) {
        ...
        const edges = rows.map(instance => this.makeEdge(base64_1.toBase64(limits.offset++), this.gqlFromDao(instance)));
    ...
    async getOne(oid) {
        ...
        return this.gqlFromDao(instance);
    }
    async create(data) {
    ...
    async getAssociatedMany(source, assoc_key, filterBy, assocApiClass, assocEdgeClass, assocConnectionClass) {
        ...
        edges = associated.map(instance => {
            ...
            edge.node = model_to_class_1.modelToClass(this.getServiceFor(assocApiClass), assocApiClass, instance);
            return edge;
        });
    ...
    async getAssociated(source, assoc_key, assocApiClass) {
        ...
        if (associatedModel) {
            Reflect.set(source, assoc_key, model_to_class_1.modelToClass(this.getServiceFor(assocApiClass), assocApiClass, associatedModel));
