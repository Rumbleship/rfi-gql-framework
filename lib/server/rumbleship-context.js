...
class RumbleshipContext {
    constructor(id, container, logger, authorizer, beeline, marshalled_trace, linked_span) {
        this.id = id;
        ...
        this.linked_span = linked_span;
        const hydrated_trace = this.beeline.unmarshalTraceContext(this.marshalled_trace);
        const trace_context = {
            name: 'rumbleship_context'
        };
        this.trace = this.beeline.startTrace(trace_context, hydrated_trace.traceId, hydrated_trace.parentSpanId, hydrated_trace.dataset);
        if (this.linked_span) {
    ...
    static make(filename, options, factories = RumbleshipContext._serviceFactories) {
        ...
        const { authorizer, container, id, logger, marshalled_trace, linked_span } = new RumbleshipContextOptionsWithDefaults(filename, options);
        ...
        const ctx = new RumbleshipContext(id, container, logger, authorizer, beeline, marshalled_trace, linked_span);
