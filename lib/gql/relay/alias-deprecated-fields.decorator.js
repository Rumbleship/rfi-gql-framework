...
function AliasFromDeprecatedField(deprecated_prop_name, field_options = {}) {
    return (target_class, new_prop_name) => {
        var _a;
        const map = (_a = Reflect.getMetadata(AliasDeprecatedFieldMap, target_class)) !== null && _a !== void 0 ? _a : new Map();
        ...
        return type_graphql_1.Field(field_options)(target_class, new_prop_name);
    };
}
...
function cloneAndTransposeDeprecatedValues(filterOrInputType) {
    ...
    if (map) {
        ...
        for (const [deprecated_field_prop_name, new_prop_name] of map.entries()) {
            ...
            delete cloned[`__${new_prop_name}`];
        }
        return cloned;
    }
