...
let ClassGqlNodeNotification = class ClassGqlNodeNotification extends node_notification_1.NodeNotification {
    constructor(notificationOf, node, deltas) {
...
__decorate([
    type_graphql_1.Field(type => [node_notification_1.GqlModelDelta], { nullable: true }),
    __metadata("design:type", Array)
], ClassGqlNodeNotification.prototype, "deltas", void 0);
ClassGqlNodeNotification = __decorate([
    type_graphql_1.ObjectType(),
    __metadata("design:paramtypes", [String, index_1.Node, Array])
...
let NodeResolver = class NodeResolver {
    ...
    async onChange(payload) {
        ...
        const gqlModelName = payload.model.constructor.name.slice(0, payload.model.constructor.name.length - 'Model'.length);
        const oid = oid_1.Oid.Create(gqlModelName, modelId);
        if (gqlModelName in this.nodeServices) {
            const node = Reflect.get(this.nodeServices, gqlModelName).getOne(oid);
            const gqlNodeNotification = new ClassGqlNodeNotification(payload.notificationOf, node, payload.deltas);
