...
async function createNodeNotification(raw, resolver, NotificationType, delta_keys) {
    ...
    return ctx.beeline.bindFunctionToTrace(async () => {
        return ctx.beeline.withAsyncSpan({ name: 'createPayload' }, async (_span) => {
            ...
            const marshalled = o11y_1.RumbleshipBeeline.marshalTraceContext(ctx.beeline.getTraceContext());
            const gql_node_notification = new NotificationType(received.action, received.idempotency_key, node, modeldeltas, marshalled);
