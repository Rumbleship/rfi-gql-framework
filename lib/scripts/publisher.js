...
class Publisher {
    constructor() {
        this.message_one_received = false;
        this.message_two_received = false;
        this.count = 0;
        this.pubsub = new pubsub_1.PubSub();
    }
    orderedTopic() {
        return queued_graphql_1.gcpGetTopic(this.pubsub, 'test-ordered', true);
    }
    unorderedTopic() {
        return queued_graphql_1.gcpGetTopic(this.pubsub, 'test-unordered', false);
    }
    async publishUnordered(message) {
        const topic = await this.unorderedTopic();
        await topic.publish(Buffer.from(JSON.stringify(message)));
    }
    async publishOrdered(message_data) {
    ...
    async initSubscription(topic, subscription_name, opts = {}) {
    ...
    async listenOrdered(instance_id) {
        const topic = await this.orderedTopic();
        const subscription = await this.initSubscription(topic, 'ordered-sub', {
            enableMessageOrdering: true
        });
        for await (const [message] of events_1.on(subscription, 'message')) {
            this.count++;
            const message_data = message.data.toString();
            const payload = this.parseMessage(message_data);
            this.handler(message_data, message, payload, instance_id);
    ...
    handler(message_data, message, payload, instance_id) {
    ...
    messages(subscription) {
        return events_1.on(subscription, 'message');
    }
