...
class RfiPubSubSubscription {
    ...
    async init() {
        if (!this._initiaized) {
            await this.beeline.withAsyncSpan({ name: 'RfiPubSubSubscription.init' }, async () => await this.initSubscription());
        }
    }
    async initSubscription() {
        ...
        if (!exists) {
            const topic = await helpers_1.gcpGetTopic(this._pubSub, this.gcloud_topic_name);
    ...
    async start(handler, source_name = this.constructor.name) {
        await this.beeline.withTrace({ name: 'RfiPubSubSubscription.start' }, async () => {
            await this.beeline.bindFunctionToTrace(async () => {
                ...
                let retry = true;
                while (retry) {
                    await this.beeline.withAsyncSpan({ name: 'RfiPubSubSubscription.retryLoop' }, async () => {
                        ...
                        this.beeline.finishSpan(this.beeline.startSpan({ name: 'RfiPubSubSubscription.messageLoopStarting' }));
                        try {
                            ...
                            for await (const [message] of this.messages) {
                                ...
                                if (payload) {
                                    ...
                                    await this.beeline.runWithoutTrace(() => handler(ctx, payload)
                                        .catch(error => {
                            ...
                            this.beeline.finishSpan(this.beeline.startSpan({ name: 'RfiPubSubSubscription.messageLoopStopped' }));
                            ...
                            this.logger.error(`Exception in message loop for ${source_name} : ${this.gcloud_topic_name}, subscription: ${this.gcloud_subscription_name}`, {
