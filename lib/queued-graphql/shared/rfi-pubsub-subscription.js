...
class RfiPubSubSubscription {
    ...
    start(handler, source_name = this.constructor.name) {
        ...
        const initStartAndIterate = async () => {
            ...
            return await this.beeline.withAsyncSpan({ name: 'RfiPubSubSubscription.iterate' }, async () => {
                return await this.iterate(handler, source_name);
            });
        };
        const wrapped = this.beeline.bindFunctionToTrace(() => initStartAndIterate().finally(() => this.beeline.finishTrace(trace)));
        return wrapped().then(should_restart => should_restart ? this.start(handler, source_name) : undefined);
    }
    /**
     *
     * @param handler
