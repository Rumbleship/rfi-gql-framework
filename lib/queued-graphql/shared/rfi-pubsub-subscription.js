...
class RfiPubSubSubscription {
    ...
    async start(handler, source_name = this.constructor.name) {
        ...
        let retry = true;
        while (retry) {
            this.logger.info(`RfiPubSubSubscription: Starting message loop for ${this.constructor.name} : ${this.gcloud_topic_name}, subscription: ${this.gcloud_subscription_name}`);
            try {
