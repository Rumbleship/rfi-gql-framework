...
const queued_subscription_observer_1 = require("./queued_subscription_observer");
/**
...
class QueuedSubscriptionObserverManager {
    ...
    setHandlers(observers) {
        this.handlers = new Map(observers
            .map(observer => queued_subscription_observer_1.getQsoHandlers(observer))
            .flat()
            .map(handler => [handler.qso_metadata.subscription_name, handler]));
    }
    async init(ctx, observers) {
    ...
    async syncQsrs(ctx) {
        ...
        for (const handler of this.handlers.values()) {
            const gql_query_string = typeof handler.qso_metadata.gql_document === 'string'
                ? handler.qso_metadata.gql_document
                : graphql_1.print(handler.qso_metadata.gql_document);
