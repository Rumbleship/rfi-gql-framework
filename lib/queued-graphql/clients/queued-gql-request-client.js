...
class QueuedGqlRequestClientOneInstanceResponder {
    constructor(config) {
        ...
        this._response_subscription = new rfi_pubsub_subscription_1.RfiPubSubSubscription(config, this._pubsub, this.request_topic_name, this.response_subscription_name);
    }
    ...
    async start() {
        ...
        await this._response_subscription.start(async (response, ctx) => {
