...
exports.QueuedSubscriptionServer = exports.QUEUED_SUBSCRIPTION_REPO_CHANGE_GQL = exports.QSR_GQL_FRAGMENT = exports.QUEUED_SUBSCRIPTION_REPO_CHANGE_TOPIC = void 0;
...
exports.QUEUED_SUBSCRIPTION_REPO_CHANGE_GQL = `
    ...
    ${exports.QSR_GQL_FRAGMENT}
    `;
function buildPrimeQsrCacheListQuery({ first, after } = { first: 100 }) {
    return `query qsrs {
        queuedSubscriptionRequests(
          order_by: { keys: [["cache_consistency_id", "ASC"]] }
          first: ${first}
          after: "${after !== null && after !== void 0 ? after : ''}"
        ) {
          edges {
            node {
              ...qsr
...
class QueuedSubscriptionServer {
     ...
     */
    async addSubscriptionAndStart(ctx, key, request) {
        addQsoToTraceMetadata(ctx.beeline, request);
        if (this.queuedSubscriptions.has(key)) {
    ...
    async handler_GetAllQueuedSubscriptionRequests(ctx, response) {
        ...
        if (response.response.data) {
            ...
            const qsrs = edges.map(entry => entry.node);
            if (qsrs.length) {
        ...
        if (response.response.errors) {
            for (const error of response.response.errors) {
                ctx.logger.error(`Error in response: ${graphql_1.printError(error)}`);
                ctx.beeline.finishSpan(ctx.beeline.startSpan({
                    name: 'error',
                    error: {
                        ...error,
                        plain: graphql_1.printError(error),
                        stack: (_a = error.originalError) === null || _a === void 0 ? void 0 : _a.stack,
                        message: (_b = error.originalError) === null || _b === void 0 ? void 0 : _b.message
                    }
                }));
            }
        }
...
function addQsoToTraceMetadata(beeline, qso) {
    const { owner_id, marshalled_acl, publish_to_topic_name, create_unique_subscription, serviced_by, deleted_at, cache_consistency_id, gql_query_string, query_attributes, operation_name, subscription_name, active, id } = qso;
