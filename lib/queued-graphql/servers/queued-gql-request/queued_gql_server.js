...
class QueuedGqlRequestServer {
    ...
    async start(ctx) {
        ...
        this._request_subscription.start(async (ctx, request) => {
            ...
            try {
                const executionParams = ctx.beeline.bindFunctionToTrace(() => QueuedGqlRequestServer.validateGqlRequest(this.schema, request))();
                ...
                if (request.respond_on_error) {
                    const gqlError = error instanceof graphql_1.GraphQLError
                        ? error
                        : new graphql_1.GraphQLError((_a = error.message) !== null && _a !== void 0 ? _a : 'error during request execution', undefined, undefined, undefined, undefined, error);
    ...
    async publishResponse(ctx, request, executionResponse) {
        ...
        const topic = await helpers_1.gcpGetTopic(this._pubsub, request.publish_to_topic_name, false);
    ...
    isRequestSyncQsrs(request) {
        return !!request.client_request_id.match(/\w+\.syncQsrs/);
    }
    shouldProcessSyncRequest() {
    ...
    static validateGqlRequest(schema, subscriptionRequest) {
        ...
        if (errors.length) {
            const errString = JSON.stringify(errors.map(error => graphql_1.printError(error)), undefined, 2);
