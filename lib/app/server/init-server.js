...
async function initServer(config, InjectedBeeline, injected_hapi_plugins, injected_apollo_server_options = {}, injected_models, injected_schema_options, injected_routes = [], onContainer, onInitialized = (_server, _pubSub) => Promise.resolve(), dbOptions) {
    ...
    const pubSub = new rfi_pub_sub_engine_1.RfiPubSub(config.gae_version, config.PubSubConfig, config.gcp.auth, InjectedBeeline);
        ...
        subscriptions: {
            onConnect: (connectionParams, webSocket, context) => {
                ...
                if (bearer_token) {
                    const authorizer = (() => {
                        try {
                            return new acl_1.Authorizer(bearer_token, config.internalAuthConfig.access_token_secret);
                        }
                        catch (error) {
