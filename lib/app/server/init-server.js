...
async function initServer(config, InjectedBeeline, injected_hapi_plugins, injected_apollo_server_options = {}, injected_models, injected_schema_options, injected_routes = [], onContainer, onInitialized = (_server, _pubSub) => Promise.resolve(), dbOptions) {
    ...
    const to_inject = injected_models.find(entry => entry.scope === queued_graphql_1.QsrCacheOidScope)
        ? [...injected_models, ...[queued_graphql_1.QueuedCacheScopeAndDb]]
        : injected_models;
    const sequelize = await init_sequelize_1.initSequelize(config.Db, msg => serverLogger.debug(msg), to_inject, dbOptions);
