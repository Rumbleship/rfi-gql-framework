...
async function initServer(config, injected_plugins, injected_models, injected_schema_options, injected_routes = [], onContainer, onInitialized = (_server) => Promise.resolve(), dbOptions) {
    ...
    const server = o11y_1.RumbleshipBeeline.shimFromInstrumentation(new Hapi.Server(serverOptions));
    const sequelize = await init_sequelize_1.initSequelize(config.db, msg => serverLogger.debug(msg), injected_models, dbOptions);
    await sequelize.authenticate();
    const pubSub = new rfi_pub_sub_engine_1.RfiPubSub(config.gae_version, config.PubSubConfig, o11y_1.RumbleshipBeeline);
    ...
    const apolloServer = new apollo_server_hapi_1.ApolloServer({
        ...
        context: async (ctx, _connection) => {
            ...
            if (container && onContainer) {
                ...
                onContainer(container, rumbleship_context, service_factory_map_1.ServiceFactories);
            }
