...
const watch_connection_pool_1 = require("./watch-connection-pool");
...
const add_error_to_trace_context_1 = require("../honeycomb-helpers/add_error_to_trace_context");
const init_queued_graphql_1 = require("../../queued-graphql/init_queued_graphql");
async function initServer(config, auth0Config, InjectedBeeline, injected_hapi_plugins, injected_apollo_server_options = {}, injected_models, injected_schema_options, injected_routes = [], onContainer, onInitialized = (_server, _pubSub) => Promise.resolve(), dbOptions) {
    acl_1.Auth0Authorizer.initialize(config, auth0Config);
    ...
    if (config.traceDatabaseConnectionHooks) {
        watch_connection_pool_1.attachConnectionHooks();
    }
        ...
        subscriptions: {
            onConnect: async (connectionParams, webSocket, context) => {
                ...
                if (bearer_token) {
                    const authorizer = (() => {
                        try {
                            return acl_1.Auth0Authorizer.make(bearer_token, true);
                        }
                        catch (error) {
    ...
    init_queued_graphql_1.initQueuedGraphql(config, exports.globalGraphQlSchema, injected_schema_options.resolvers);
    server.events.on('start', async () => {
        ...
        try {
            ...
            //
            await init_queued_graphql_1.startQueuedGraphQl(ctx);
        }
        catch (error) {
    ...
    server.events.on('stop', async () => {
        ...
        try {
            await init_queued_graphql_1.stopQueuedGraphQl(ctx);
        }
        catch (error) {
            add_error_to_trace_context_1.addErrorToTraceContext(ctx, error);
    ...
    server.route([health_checks_route_1.root_route, health_checks_route_1.health_check_route, ...injected_routes]);
