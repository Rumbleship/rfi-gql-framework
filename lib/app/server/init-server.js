...
const servers_1 = require("../../queued-graphql/servers");
async function initServer(config, InjectedBeeline, injected_hapi_plugins, injected_apollo_server_options = {}, injected_models, injected_schema_options, injected_routes = [], onContainer, onInitialized = (_server, _pubSub) => Promise.resolve(), dbOptions) {
    ...
    const queuedSubscriptionServer = new servers_1.QueuedSubscriptionServer(config, exports.globalGraphQlSchema);
    const queuedGqlRequestServer = new servers_1.QueuedGqlRequestServer(config, exports.globalGraphQlSchema);
