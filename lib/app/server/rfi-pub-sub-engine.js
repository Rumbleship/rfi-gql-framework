...
class RfiPubSub extends graphql_google_pubsub_1.GooglePubSub {
    constructor(publisher_version, config, beeline) {
    ...
    linkToSequelize(sequelize) {
        ...
        sequelize.afterCreate(hookCb(gql_1.NotificationOf.CREATED));
        sequelize.afterUpdate(hookCb(gql_1.NotificationOf.UPDATED));
        // sequelize.afterBulkCreate((instances, options) => gqlBulkCreateHook(pubSub, instances, options));
    }
    getMarshalledTraceContext(trace_id) {
        return this.beeline_cls.prototype.bindFuntionToTrace(() => this.beeline_cls.prototype.getContext(), trace_id)();
    }
