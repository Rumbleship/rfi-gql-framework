...
class RfiPubSub extends graphql_google_pubsub_1.GooglePubSub {
        ...
        const hookCb = (pubSub, notification_of) => {
            return function publisherHook(sequelize_instance, options) {
                ...
                if (options && options.transaction) {
                    ...
                    const context_id = rumbleship_context_1.getContextId(outerTransaction);
                    const authorized_user = rumbleship_context_1.getAuthorizedUser(outerTransaction);
                    outerTransaction.afterCommit(t => {
    ...
    static getOutermostNestedTransaction(transaction) {
