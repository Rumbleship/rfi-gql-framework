import { PubSub as GooglePubSub, PubSub, Subscription } from '@google-cloud/pubsub';
...
import { IQueuedGqlRequest, IQueuedGqlResponse } from './queued-gql-request.interface';

import { RfiPubSubSubscription } from './rfi-pubsub-subscription';
...
export class QueuedGqlRequestServer {
  ...
  constructor(public config: ISharedSchema, private schema: GraphQLSchema) {
    this.request_topic_name = `${config.PubSub.topicPrefix}_GRAPHQL_REQUEST`;
    ...
    this._pubsub = new PubSub(config.Gcp.Auth);
